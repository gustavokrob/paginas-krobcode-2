...
`DEFAULT_COUNTRY_CODE` (default `55`; only matters if you add a phone field),
...
npx wrangler@latest d1 migrations apply <project>-db --remote   # applies 0001..0016
```
...
should see `Lead` and `Lead31Plus` with Advanced Matching populated and pixel↔CAPI
deduped by `event_id`. The dashboard is at `https://<your-domain>/dash/?key=<DASH_KEY>`.
